...
# ── Synthetic pair generator (inline) ───────────────────────────────────────

def make_pair(dx: float, dy: float, angle_deg: float,
              size=(256, 256), noise_sigma=1.0, seed=42):
    """Return (reference, transformed, ground_truth_dict)."""
    rng = np.random.default_rng(seed)
    ref = rng.integers(40, 220, size, dtype=np.uint8).astype(np.float32)
    # add some structure so ECC has something to grip
    for _ in range(30):
...
TRANSLATION_TOL = 0.1   # px  — hard limit
ROTATION_TOL    = 0.05  # deg — hard limit
GOOD_CONFIDENCE = 0.6
